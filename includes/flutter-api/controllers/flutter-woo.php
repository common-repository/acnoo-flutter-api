...
class FlutterWoo extends FlutterBaseController {
	...
	public function register_flutter_woo_routes() {
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => 'GET',
					'callback'            => array( $this, 'get_product_category_list' ),
					'permission_callback' => function () {
						return parent::checkApiPermission();
					},
				),
					...
					for ( $i = 0; $i < $count; $i++ ) {
						...
						if ( is_plugin_active( 'wc-frontend-manager-delivery/wc-frontend-manager-delivery.php' ) ) {
							...
							if ( count( $users ) > 0 ) {
								...
								$order['line_items'][ $i ]['delivery_user'] = array(
									'id'              => $user->ID,
	...
	public function coupon( $request ) {
		...
		WC()->cart->set_applied_coupons( array( $coupon_code ) );
	...
	function get_products_video( $request ) {
		...
		$items = $wpdb->get_results( "SELECT * FROM $table_name WHERE meta_key='_maanstore_video_url' AND meta_value IS NOT NULL AND meta_value <> '' LIMIT $per_page OFFSET $page" );
	...
	function get_product_category_list( $request ) {
		$token   = sanitize_text_field( $request['token'] );
		$user_id = validateCookieLogin( $token );
		if ( is_wp_error( $user_id ) ) {
			return $user_id;
		}
		...
		if ( ! array_intersect( $current_user_role, $allowed_roles ) ) {
			return parent::sendError( 'unauthorized', 'You need higher permission to do this', 401 );
		}
	...
	function get_product_list( $request ) {
		...
		$posts_per_page = intval( $request['posts_per_page'] ) ?? 10;
		$paged          = intval( $request['page_number'] ) ?? 0;
		...
		return new WP_REST_Response(
			array(
				'status'   => 'success',
				'response' => array(
					'product_list' => $product_query->posts,
				),
			),
		);
	}
	...
	function get_on_sale_products( $request ) {
		$token   = sanitize_text_field( $request['token'] );
		$user_id = validateCookieLogin( $token );
		if ( is_wp_error( $user_id ) ) {
			return $user_id;
		...
		$on_slae_product_query = new WP_Query( $query_args );

		return new WP_REST_Response(
			array(
				'status'   => 'success',
				'response' => array(
					'product_list' => $on_slae_product_query->posts,
				),
			),
		);
	}
