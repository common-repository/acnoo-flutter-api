...
class FlutterUserController extends FlutterBaseController {
	...
	public function register() {
		...
		if ( isset( $role ) ) {
			if ( ! in_array( $role, array( 'subscriber', 'wcfm_vendor', 'seller', 'wcfm_delivery_boy', 'driver', 'owner' ), true ) ) {
		...
		if ( ! validate_username( $username ) ) {
			...
			return parent::sendError( 'existed_username', 'Username already exists.', 403 );
		} else {
			if ( ! is_email( $email ) ) {
	...
	function update_user_profile() {
		...
		if ( isset( $params->deviceToken ) ) {
			if ( isset( $params->is_manager ) && $params->is_manager ) {
				update_user_meta( $user_id, 'maanstore_manager_device_token', $params->deviceToken );
			} elseif ( isset( $params->is_delivery ) && $params->is_delivery ) {
				update_user_meta( $user_id, 'maanstore_delivery_device_token', $params->deviceToken );
			}
			if ( ! isset( $params->is_delivery ) && ! isset( $params->is_manager ) ) {
				update_user_meta( $user_id, 'maanstore_device_token', $params->deviceToken );
			}
	...
	public function test_push_notification() {
		...
		if ( $is_onesignal ) {
			$status = one_signal_push_notification( 'MaanStore', 'Test push notification', array( $user_id ) );
			return array( 'status' => $status );
		}
		if ( isset( $is_manager ) ) {
			if ( $is_manager ) {
				$deviceToken = get_user_meta( $user_id, 'maanstore_manager_device_token', true );
				if ( $deviceToken ) {
		...
		if ( isset( $is_delivery ) ) {
			if ( $is_delivery ) {
				$deviceToken = get_user_meta( $user_id, 'maanstore_delivery_device_token', true );
				if ( $deviceToken ) {
					$status = pushNotification( 'MaanStore', 'Test push notification', $deviceToken );
				}
			}
		...
		$deviceToken = get_user_meta( $user_id, 'maanstore_device_token', true );
		if ( $deviceToken ) {
			$status = pushNotification( 'MaanStore', 'Test push notification', $deviceToken );
		}
		return array(
	...
	function chat_notification() {
		...
		$deviceToken          = get_user_meta( $receiver->ID, 'maanstore_device_token', true );
		$manager_device_token = get_user_meta( $receiver->ID, 'maanstore_manager_device_token', true );
		pushNotification( $sender_name, $message, $deviceToken );
		pushNotification( $sender_name, $message, $manager_device_token );
	}

	function maanstore_digrest_set_variables() {
		$json   = file_get_contents( 'php://input' );
		$params = json_decode( $json, true );
	...
	function digits_register() {
		...
		$this->maanstore_digrest_set_variables();
